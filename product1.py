...
class Productclass:
    def __init__(self,root):
      ...
      #======column1
        ...
        lbl_product_name=Label(product_Frame,text="Name",font=("goudy old style",15),bg="white").place(x=30,y=110)
        lbl_Price=Label(product_Frame,text="Price",font=("goudy old style",15),bg="white").place(x=30,y=160)
        lbl_qty=Label(product_Frame,text="Quantity",font=("goudy old style",15),bg="white").place(x=30,y=210)
        lbl_Status=Label(product_Frame,text="Status",font=("goudy old style",15),bg="white").place(x=30,y=260)
        lbl_customer=Label(product_Frame,text="Coustmor",font=("goudy old style",15),bg="white").place(x=30,y=310)
         #======column2
        ...
        cmb_cust=ttk.Combobox(product_Frame,textvariable=self.var_cust,values=self.cust_list,state='readonly',justify=CENTER,font=("goudy old style",15))
        cmb_cust.place(x=150,y=310,width=200)
        cmb_cust.current(0)
       
        #===buttons===
        btn_add=Button(product_Frame,text="Save",command=self.add,font=("goudy old style",15),bg="#2196f3",fg="white",cursor="hand2").place(x=10,y=350,width=100,height=40)
        ...
        btn_clear=Button(product_Frame,text="Clear",command=self.clear,font=("goudy old style",15),bg="#607d8b",fg="white",cursor="hand2").place(x=340,y=350,width=100,height=40)
     #====searchfram====
     ...
     #=========product detels
        ...
        self.productTable=ttk.Treeview(p_frame,columns=("pid","cat","name","price","qty","status"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
          ...
          if self.var_cat.get()=="Select" or self.var_cat.get()=="Empty" or self.var_cust.get()=="Select"or self.var_name.get()=="":
            ...
            if row!=None:
              messagebox.showerror("Error","Product already present,try different",parent=self.root)
                ...
                cur.execute("Insert into product (cat,name,price,qty,status) values(?,?,?,?,?)",(
                                    self.var_cat.get(),
                                    self.var_name.get(),
                                    self.var_price.get(),
                                    self.var_qty.get(),
          ...
          if self.var_pid.get()=="":
            ...
            if row==None:
              messagebox.showerror("Error","Invalide Product",parent=self.root)
                ...
                cur.execute("Update product set cat=?,name=?,price=?,qty=?,status=? where pid=?",(
